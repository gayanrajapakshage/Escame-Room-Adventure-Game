...
public class Room implements Serializable {
    ...
    /**
     * The list of NPCs in the room.
     */
    public ArrayList<NPC> NPCsInRoom = new ArrayList<NPC>();
    ...
    public String getObjectString() {
        ...
        for (AdventureObject thisObject: this.objectsInRoom){
            resultList.add(thisObject.getDescription());
        }
    ...
    public String getCommands() {
        ArrayList<String> resultList = new ArrayList<>();
        for (Passage thisPassage: this.motionTable.passageTable){
            if (!resultList.contains(thisPassage.getDirection())){
                resultList.add(thisPassage.getDirection());
            }
        }
        return String.join(", ", resultList);
    }
    ...
    public String getNPCstring(){
        ArrayList<String> resultList = new ArrayList<>();
        for (NPC npc: this.NPCsInRoom){
            String[] fullname = npc.name.split(":");
            resultList.add(fullname[0]);
        }
        return String.join(", ", resultList);
    }

    /**
     * Removes the NPC from room
     * @param npc the NPC
     */
    public void removeNPC(NPC npc){
        for(int i = 0; i<NPCsInRoom.size();i++){
            if(this.NPCsInRoom.get(i).equals(npc)) this.NPCsInRoom.remove(NPCsInRoom.get(i));
        }
    }
